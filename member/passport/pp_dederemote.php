...
作者： IT柏拉图  最后修改日期 2007-3-29
//**********************************************************************
本文件仅作为网关供远程调用
请使用或参考 pp_dederemote_interface.php 提供的接口函数进行整合编程
-----------------------------------------*/
  ...
  if(strlen($userpwd)>24){
	...
	if(is_array($row)){
	   ...
	   if($userpwd != $pwd){
	      ...
	      $backString = "DedeUserID=$ID&DedeLoginTime=$ntime";
	      echo 'OK!'.base64_encode($backString);
	      exit();
	   }
	}
	$userpwd = GetEncodePwd($userpwd);
...
---------------------------------*/
	...
	$backString = "DedeUserID=0&DedeLoginTime=0";
	echo 'OK!'.base64_encode($backString);
	exit();
}
/*--------------------------------
