...
class userLogin
{
    ...
    function checkUser($username, $userpwd)
    {
        ...
        $dsql->SetQuery("SELECT `admin`.*, `atype`.`purviews` FROM `#@__admin` `admin` LEFT JOIN `#@__admintype` `atype` ON `atype`.`rank` = `admin`.`usertype` WHERE `admin`.`userid` = '{$this->userName}'");
        $dsql->Execute();
        $row = $dsql->GetObject();
        if(!isset($row->pwd))
        {
