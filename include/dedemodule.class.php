...
class DedeModule
{
    ...
    public $sysLang;
    public $moduleLang;
    public function __construct($modulespath = '', $modulesUrl = '')
    {
    ...
    public function DedeModule($modulespath = '')
    {
    ...
    public function GetHashFile($hash)
    {
        ...
        if (isset($GLOBALS['allmodules'][$hash])) {
            return $GLOBALS['allmodules'][$hash];
        } else {
            return $hash . '.xml';
        }
    ...
    public function GetModuleInfo($hash, $ftype = 'hash')
    {
        ...
        if (isset($minfos['lang'])) {
            $this->moduleLang = trim($minfos['lang']);
        } else {
            $this->moduleLang = 'gbk';
        }

        if ($this->sysLang == 'gb2312') {
            $this->sysLang = 'gbk';
        }

        if ($this->moduleLang == 'gb2312') {
            $this->moduleLang = 'gbk';
        }

        if ($this->sysLang != $this->moduleLang) {
            foreach ($minfos as $k => $v) {
                $minfos[$k] = $this->AppCode($v);
            }
    ...
    public function WriteFiles($hashcode, $isreplace = 3)
    {
        ...
        while (!feof($fp)) {
            ...
            if (preg_match("/^[\s]{0,}<file/i", $line)) {
                ...
                $filename = $dap->CAtt->GetAtt('name');
                $filename = str_replace("\\", "/", $filename);
                if (!empty($AdminBaseDir)) {
                    $filename = $AdminBaseDir . $filename;
                }
    ...
    public function MakeEncodeFileRunTest($basedir, $f)
    {
        ...
        if (isset($this->fileListNames[$f])) {
            return;
        } else if (preg_match("/Thumbs\.db/i", $f)) {
            return;
        } else {
