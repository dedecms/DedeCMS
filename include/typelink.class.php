...
class TypeLink
{
    ...
    public function TypeLink($typeid)
    {
    ...
    //获得某类目的链接列表，递归逻辑部分
    public function LogicGetPosition($id, $islink)
    {
    ...
    //获得某个类目的超链接信息
    public function GetOneTypeLink($typeinfos)
    {
    ...
    //获得某分类连接的URL
    public function GetOneTypeUrl($typeinfos)
    {
        if (defined('DEDEMOB')) {
    ...
    public function GetOptionArray($hid = 0, $oper = 0, $channeltype = 0, $usersg = 0)
    {
        return $this->GetOptionList($hid, $oper, $channeltype, $usersg);
    }

    public function GetOptionList($hid = 0, $oper = 0, $channeltype = 0, $usersg = 0)
    {
        global $cfg_admin_channel;
        if (empty($cfg_admin_channel)) {
        ...
        if ($channeltype == 0) {
            ...
            $ctsql = " AND channeltype='$channeltype' ";
        }
