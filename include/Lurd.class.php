...
{
    // 固定属性
    public $dateTypes = array('DATE', 'DATETIME', 'TIMESTAMP', 'TIME', 'YEAR');
    public $floatTypes = array('FLOAT', 'DOUBLE', 'DECIMAL');
    public $intTypes = array('TINYINT', 'SMALLINT', 'MEDIUMINT', 'INT', 'BIGINT');
    public $charTypes = array('VARCHAR', 'CHAR', 'TINYTEXT');
    public $textTypes = array('TEXT', 'MEDIUMTEXT', 'LONGTEXT');
    public $binTypes = array('TINYBLOB', 'BLOB', 'MEDIUMBLOB', 'LONGBLOB', 'BINARY', 'VARBINARY');
    public $emTypes = array('ENUM', 'SET');
    // 普通属性
    public $tableName = '';
    public $templateDir = '';
    public $lurdTempdir = '';
    ...
    // 单条记录管理,修改/删除
    public $singleManage = true;

    //构造函数，指定要操作的表名
    public function __construct($tablename, $templatedir = '', $lurdtempdir = '')
    {
        global $dsql;
        $prefix = "#@__";
    ...
    public function ListenAll($listfield = '', $wherequery = '', $orderquery = '')
    {
        global $action;
    ...
    public function AddPriKey($fieldname)
    {
        $this->primaryKey = $fieldname;
    }

    // 指定应用的名称
    public function AddAppName($appname)
    {
        $this->appName = $appname;
    }

    //设置用于排序的SQL，如 order by id desc
    public function SetOrderQuery($query)
    {
        $this->orderQuery = $query;
    }

    //强制指定字段为其它类型
    public function BindType($fieldname, $ftype, $format = '')
    {
    ...
    public function BindTemplate($fieldname, $tmptype = 'list', $temp = '')
    {
     ...
     **********************************/
    public function ListData($listfield = '*', $wherequery = '', $orderquery = '', $Suff = '_list.htm')
    {
        $listdd = '';
        if (trim($listfield) == '') {
            $listfield = '*';
    ...
    public function SaveCurUrl()
    {
        setcookie('LURD_GOBACK_URL', $this->GetCurFullUrl(), time() + 3600, '/');
    }

    public function GetCurFullUrl()
    {
    ...
    //生成全部模板
    public function MakeAllTemplate($listfield = '')
    {
        $this->MakeListTemplate($listfield);
    ...
    //生成列表模板
    public function MakeListTemplate($listfield = '')
    {
    ...
    //生成发布或编辑模板
    public function MakeAddEditTemplate($getTemplets = 'add')
    {
        ...
        foreach ($this->fields as $k => $v) {
            ...
            //获得字段默认值（编辑时从数据库获取）
            if ($getTemplets == 'edit') {
                if (in_array($this->fields[$k]['type'], $this->binTypes)) {
                    $dfvalue = '';
                } else {
                    $dfvalue = "{dede:field name='$k' /}";
                }
    ...
    // 保存新增数据
    public function SaveAddData($isturn = true)
    {
        $allfield = $allvalue = '';
        foreach ($this->fields as $k => $v) {
            //自动递增键不处理
    ...
    // 保存编辑数据
    public function SaveEditData($isturn = true)
    {
        $editfield = '';
        foreach ($this->fields as $k => $v) {
            //自动递增键不处理
    ...
    // 删除数据
    public function DelData($isturn = true)
    {
        $GLOBALS[$this->primaryKey] = isset($GLOBALS[$this->primaryKey]) ? $GLOBALS[$this->primaryKey] : request($this->primaryKey);
        if (preg_match("#,#", $this->primaryKey)) {
    ...
    //$mylinkid 当前表用于联结的字段
    //$linkid 目标表用于联结的字段
    public function AddLinkTable(&$tablelurd, $mylinkid, $linkid, $linkfields = '*')
    {
        if (trim($linkfields) == '') {
            $linkfields = '*';
        }
     ...
     */
    public function AddSearchParameter($fieldname, $fieldvalue, $condition, $linkmode = 'AND')
    {
    ...
    //对请求的字符串进行安全处理
    public function StringSafe($str, $safestep = -1)
    {
    ...
    public function GetBinData($fname)
    {
    ...
    //格式化浮点数字
    public function FormatFloat($fvalue, $ftype = '')
    {
        if ($ftype == '') {
            $ftype = '%0.4f';
        }
    ...
    //获得默认时间格式
    public function GetDateTimeDf($ftype)
    {
        if ($ftype == 'DATE') {
            return 'Y-m-d';
        } else if ($ftype == 'TIME') {
    ...
    //格式化日期
    public function FormatDate($fvalue, $ftype, $fformat = '')
    {
