...
class Archives
{
    ...
    public function ParAddTable()
    {
        ...
        if (isset($this->Fields['litpic'])) {
            if ($this->Fields['litpic'] == '-' || $this->Fields['litpic'] == '') {
                $this->Fields['litpic'] = $GLOBALS['cfg_cmspath'] . '/resources/img/defaultpic.gif';
            }
    ...
    //获得当前字段参数
    public function GetCurTag($fieldname)
    {
    ...
    public function display()
    {
        ...
        if ($this->NowPage > 1) {
            $this->Fields["title"] = $this->Fields["title"] . "({$this->NowPage})";
        }
