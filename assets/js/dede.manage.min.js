...
$(this).html(function(i, text) {return text.replace(/{ /g, "{").replace(/</g, "&lt;").replace(/>/g, "&gt;").replace(/"/g, "&quot;").replace(/'/g, "&#039;");});
...
$uploadCrop.croppie('setView','225','300');
});

// 4:3
$('.upload-4-3').on('click', function () {  
$uploadCrop.croppie('setView','400','300');
});

// 16:9
$('.upload-16-9').on('click', function () {  
$uploadCrop.croppie('setView','533','300');
});
// 16:9
$('.upload-512-120').on('click', function () {  
$uploadCrop.croppie('setView','512','120');
});

// 头像
$('.upload-1-1').on('click', function () {  
$uploadCrop.croppie('setView','300','300');
});

// 原图
$('.upload-result').on('click', function (ev, cropData) {
    resp = $('#image-crop img').attr('src')
...
// 等比切图
$('.upload-result-viewport').on('click', function (ev) {
...
// 无损切图
$('.upload-result-original').on('click', function (ev) {
$uploadCrop.croppie('result', {
type: 'canvas',
size: 'original'
}).then(function (resp) {
...
"archives_do.php?dopost=moveArchives&channelid=" + cid + "&rnd=" + Math.random() + "&qstr="
);
}
...
// Modal
function _DedeCMS_JSCODE_Version_Open_Modal_Windows(url, title) {
$('#archives').attr('src', url);
UIkit.modal("#modal-archives").show();
...
function _DedeCMS_JSCODE_Version_Delete_Atlas_Upload_Images(dede, flie) {
$.get('upload.php?dopost=delold&picfile=' + flie, function () {
}).done(function (data) {
$(dede).parent().parent().parent().remove();
})
.fail(function () {
