...
if ($step == 1) {
       ...
       '0','$spaceSta','','$safequestion','$safeanswer','$jointime','$joinip','$logintime','$loginip'); ";
        if ($dsql->ExecuteNoneQuery($inQuery)) {
            ...
            if ($cfg_mb_spacesta == -10) {
                ...
                $url = $cfg_basehost . (empty($cfg_cmspath) ? '/' : $cfg_cmspath) . "users-do.php?fmdo=checkMail&mid={$mid}&userhash={$userhash}&do=1";
            ...
            $dlist->SetTemplate(DEDETEMPLATE . '/plus/users-reg-complete.htm');
            $dlist->Display();
    ...
    $dlist = new DataListCP();
    $dlist->SetTemplate(DEDETEMPLATE . '/plus/users-reg-new.htm');
    $dlist->Display();
} else {
