...
function showTestWin(surl) {
  window.open(
    surl,
    "testWin",
    "scrollbars=yes,resizable=yes,statebar=no,width=600,height=450,left=100, top=100"
  );
}
...
function testRegx() {
  ...
  showTestWin(
    "co_do.php?dopost=testregx&regxurl=" +
      surl +
      "&startid=" +
      sstart +
      "&endid=" +
...
function selTrim(selfield) {
  ...
  window.open(
    "templets/co_trimrule.html?" + selfield,
    "coRule",
    "scrollbars=no,resizable=yes,statebar=no,width=320,height=180,left=" +
      posLeft +
      ", top=" +
      posTop
  );
