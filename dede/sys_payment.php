...
class Array2form
{
    ...
    public function __construct($formarray = array())
    {
    ...
    //获取一个特定项目的表单
    public function GetIterm($itermid = '', $itermtype = 1)
    {
    ...
    public function GetForm($key, $formarry = array(), $formtype = 'text')
    {
        ...
        case 'text':
            ...
            $reval_form = "<input type='text' name='{$key}' id='{$key}' class='uk-input uk-form-small uk-form-width-large' {$valstr}>";
            break;
        case 'select':
            //生成选择框
            $reval_title = $formarry['title'];
            $items = explode(',', $formarry['iterm']);
            $reval_form = "<select name='{$key}' class='uk-select uk-form-small uk-form-width-large'>";
            if (is_array($items)) {
                foreach ($items as $v) {
                    $v = trim($v);
                    if ($v == '') {
    ...
    //获取一个特定项目的表单
    public function SetDefaultTpl($tplname = '')
    {
        if (empty($tplname)) {
            $this->ArrFromTPL = '<p>~title~：~form~<small>~description~</small></p>';
        } else {
            if (file_exists($tplname)) {
                $this->ArrFromTPL = file_get_contents($tplname);
            } else {
                $this->ArrFromTPL = $tplname;
            }
