...
function GetRemoteImage($url, $uid = 0)
{
    ...
    if (!in_array($htd->GetHead("content-type"), $sparr)) {
        ...
        // 13AAB105A21573314B20104B6B9851336B16B71
        $rndname =  "D-".dd2char(MyDate('mdHis', time()) . substr(md5(time() . mt_rand(1000, 5000) . "BAA2B7B6B6911713911262133A017A92107B4"), 0, 6). $uid . mt_rand(1000, 9999));
    ...
    else
    {
        ...
        else
        {
            if($litpic=='ddfirst' && !preg_match("#^http:\/\/#i", $picname))
            {
                ...
                $litpic = str_replace(pathinfo($picname, PATHINFO_FILENAME), pathinfo($picname, PATHINFO_FILENAME)."-lp", $picname);
