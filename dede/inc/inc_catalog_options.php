...
function GetOptionList($selid = 0, $userCatalog = 0, $channeltype = 0)
{
    ...
    if ($selid > 0) {
        ...
        if ($row['ispart'] == 1) {
            $OptionArrayList .= "<option value='" . $row['id'] . "' class='option1' selected='selected'>" . $row['typename'] . "(封面频道)</option>\r\n";
        } else {
            $OptionArrayList .= "<option value='" . $row['id'] . "' selected='selected'>" . $row['typename'] . "</option>\r\n";
        }
    ...
    while ($row = $dsql->GetObject('cc')) {
        ...
        if ($sonCats != '') {
            if ($row->ispart == 1) {
                $OptionArrayList .= "<option value='" . $row->id . "' class='option1'>" . $row->typename . "(封面频道)</option>\r\n";
            } else if ($row->ispart == 2) {
                $OptionArrayList .= '';
            } else if (empty($channeltype) && $row->ispart != 0) {
                $OptionArrayList .= "<option value='" . $row->id . "' class='option2'>" . $row->typename . "(" . $channels[$row->channeltype] . ")</option>\r\n";
            } else {
