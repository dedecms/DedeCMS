...
if($nextpage==$totalpage){
	...
	if($finishType){
	  $gourl = "action_makehtml_list.php?maxpagesize=$maxpagesize&typeid=$typeid&pageno=$nextpage&uptype=$uptype&starttime=".urlencode($starttime);
	  ShowMsg("成功创建栏目：".$tid."，继续进行操作！",$gourl,0,100);
  }
  else
  {
  	$gourl = "action_makehtml_list.php?mkpage=$mkpage&maxpagesize=$maxpagesize&typeid=$typeid&pageno=$pageno&uptype=$uptype&starttime=".urlencode($starttime);
