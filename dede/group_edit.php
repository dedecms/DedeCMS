 ...
 * @version  $Id: group_edit.php,v 1.1 2009/08/04 04:06:40 blt Exp $
 */
...
//====function Save Group Info=====//
if($action=="save")
{
	$groupname = cn_substr($groupname,75);
	$storeid = ereg_replace("[^0-9]","",$store);
	$issystem = ereg_replace("[^0-1]","",$issystem);
	$ishidden = ereg_replace("[^0-1]","",$ishidden);
	if(!isset($isindex))
	{
		$isindex = $groupisindex;
	}
	$isindex  = ereg_replace("[^0-1]","",$isindex);
