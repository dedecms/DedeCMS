...
function csrf_check()
{
    ...
    if (!isset($token) || strcasecmp($token, $_SESSION['token']) !== 0) {
        echo '<a href="http://bbs.dedecms.com/907721.html">DedeCMS ERROR:CSRF Token Check Failed!</a>';
        exit;
    }
}
...
function XSSClean($val)
{
    if (is_array($val)) {
        foreach ($val as $key => $v) {
            if (in_array($key, array('tags', 'body', 'dede_fields', 'dede_addonfields', 'dopost', 'introduce'))) {
                continue;
            }
